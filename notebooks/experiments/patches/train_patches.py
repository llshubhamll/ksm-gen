...
def load_data():
    ...
    Loads Mnist data
    """
    data_path = Path.cwd().parents[2] / 'models/patches/patches_kmeans_192.pkl'
    print(data_path)
    with open(data_path, 'rb') as f:
        data_dict = pickle.load(f)
        
    patches = data_dict['data']
    init_dict = data_dict['dict']
    init_codes = data_dict['codes']
                        
    # print(data_path)
    
    return patches, init_dict, init_codes
...
@hydra.main(version_base=None, config_path="../../../configs", config_name='ksm_patches.yaml')
def main(cfg:DictConfig):
    
    exp_params = OmegaConf.to_container(cfg, resolve=True)
    
    np.random.seed(42)
    data, init_dict, init_codes = load_data()
    fraction = exp_params['dataset']['fraction']
    n_samples = int(data.shape[0] * fraction)
    positions = np.random.choice(data.shape[0], size=n_samples, replace=False)
    
    if fraction < 1:
        data = data[positions]
        init_codes = init_codes[positions]
    ...
    dataset = torch.utils.data.TensorDataset(torch.from_numpy(data).float(), torch.arange(data.shape[0]))
    ...
    # Load initial values
    init_dict = torch.from_numpy(init_dict).float().to(device)
    init_codes = torch.from_numpy(init_codes).float().to(device)
    ...
    print("===================")
    model = KSM_objective(exp_params, W_init=init_dict)
    ...
    project_name = f'ksm_patches'
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    exp_name = f"{exp_params['model']['Winit']}_{timestamp}"
    ...
    # M_init = init_dict @ init_dict.T / 2
    M_init = model.W @ model.W.T / 2
    M_init = M_init.repeat(runtime['T'], 1, 1).to(device)
    Z_init = init_codes.clone().to(device)
    # Z_init = torch.zeros_like(init_codes).to(device)
    variables, lagrange = initializeTensors(exp_params, runtime['T'], Zinit=Z_init, Minit=M_init, device=device)
    eta = 1 / power_method_svd(model.W.T, exp_params['device'])
        ...
        with torch.no_grad():
            for epoch in pbar:
                ...
                if (epoch % exp_params['optimizer']['log_interval'] == 0) or (epoch == exp_params['optimizer']['max_epochs'] - 1):
                    ...
                    save_data = {'model': model, 'exp_params': exp_params, 'loss_dict': loss_vals, 'Z': variables['Z'], 'data': data}
                ...
                if (epoch % exp_params['optimizer']['vis_interval'] == 0) or (epoch == exp_params['optimizer']['max_epochs'] - 1):
                    ...
                    fig1 = visualize_dense_dictionary(D_viz.T.cpu(), reshape=(16, 16))
                    fig2 = visualize_dense_dictionary(Dest_viz.T.cpu(), reshape=(16, 16))
